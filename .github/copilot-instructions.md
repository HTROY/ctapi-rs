...
- **并发**: `CtClient` 和 `CtList` 均实现 `Send + Sync`，支持多线程使用
  - `CtClient`：通过 `Arc<CtClient>` 在线程间共享
  - `CtList`：内部使用 `Mutex` 保护所有 FFI 调用，可通过 `Arc<CtList>` 跨线程共享
  - ⚠️ **重要**: `CtFind` 不是 `Send/Sync`，每个线程需要创建自己的 `CtFind` 实例
...
3. **生命周期**: `CtFind` 持有对 `CtClient` 的引用；`CtList` 持有 `Arc<CtClient>`，生命周期管理更安全
